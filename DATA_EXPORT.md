...
导出只读取当前账号在 Supabase 中已同步的 completed 个人训练及独立的消消乐完成记录。它不读取当前设备的本地/未同步记录，不包含对手、胜负、PK challenge 或配对对象数据。

XLSX 包含“训练记录”“逐题记录”“消消乐历史”“字段说明”。消消乐历史是独立轻量 sheet，只包含记录 ID、用户、开始/完成时间、总用时、关系数及版本；不含题目、答案、正确率、等级或逐卡数据。
